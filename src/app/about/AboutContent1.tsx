...
const translations = {
  ja: {
    ...
    exp_4_title: "RepLink",
    exp_4_period: "2026年1月 〜 現在", 
    exp_4_tech: ["Flutter", "Dart", "FastAPI", "Python"],
    exp_4_desc: "トレーニーのためのフィットネスSNS。日々のトレーニング記録（種目、重量、回数）の投稿や、JWT認証を用いたユーザー管理、フォロー機能などを実装しています。",

    exp_5_title: "Verde Via",
    exp_5_period: "2026年1月 〜 現在",
    exp_5_tech: ["Flutter", "Firebase", "Gemini API", "Google Maps"], // 仮の技術スタック
    exp_5_desc: "Genting Nature Adventuresをクライアントとした、AI×自然探索アプリ。画像認識による植物クイズやクエスト機能を通じ、若者の環境意識向上と自然との再接続を目指しています。",

    exp_6_title: "Final Year Project (FYP)",
    exp_6_period: "開始予定 (準備中)",
    exp_6_tech: ["AI", "Machine Learning", "Research"],
    exp_6_desc: "最終学年に向けて、AI・機械学習を用いた研究テーマの選定と予備調査を行っています。医療またはNLPへのAI応用を視野に入れたプロジェクトを計画中です。",
  ...
  en: {
    ...
    exp_4_title: "RepLink",
    exp_4_period: "Jan 2026 - Present",
    exp_4_tech: ["Flutter", "Dart", "FastAPI", "Python"],
    exp_4_desc: "A fitness SNS for trainees. Implemented training logs (exercise, weight, reps), user management with JWT auth, and follow features using Flutter and FastAPI.",

    exp_5_title: "Verde Via",
    exp_5_period: "Jan 2026 - Present",
    exp_5_tech: ["Flutter", "Firebase", "Gemini API", "Google Maps"],
    exp_5_desc: "Developing a nature exploration app for Genting Nature Adventures. Aims to raise environmental awareness through AI plant recognition, quizzes, and location-based quests.",

    exp_6_title: "Final Year Project (FYP)",
    exp_6_period: "Upcoming (In Preparation)",
    exp_6_tech: ["AI", "Machine Learning", "Research"],
    exp_6_desc: "Preparing for the final year research project. Currently selecting a research topic in AI/ML fields, potentially focusing on applications in healthcare or education.",
          ...
          <div className="lg:col-span-2 space-y-16">
            ...
            <section>
              ...
              <div className="space-y-12 relative border-l-2 border-gray-200 dark:border-gray-700 ml-3 md:ml-6 pl-8 md:pl-12 pb-4">
                ...
                {/* Project 4: RepLink (追加) */}
                <div className="relative">
                  <span className="absolute -left-[41px] md:-left-[57px] top-1 h-6 w-6 rounded-full border-4 border-white dark:border-gray-900 bg-orange-500"></span>
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow">
                    <div className="flex flex-col md:flex-row md:items-start gap-4 mb-4">
                      {/* 画像がない場合のプレースホルダー */}
                      <div className="w-full md:w-32 h-20 relative flex-shrink-0 bg-orange-50 dark:bg-orange-900/20 rounded-lg flex items-center justify-center">
                      ...
                      <div>
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white">{t.exp_4_title}</h3>
                        <p className="text-sm text-orange-600 dark:text-orange-400 font-medium mb-2">{t.exp_4_period}</p>
                        <p className="text-gray-600 dark:text-gray-300 text-sm leading-relaxed">{t.exp_4_desc}</p>
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-2 mt-4">
                      {t.exp_4_tech.map(tech => (
                        <span key={tech} className="px-3 py-1 text-xs font-medium text-orange-700 bg-orange-50 dark:bg-orange-900/30 dark:text-orange-300 rounded-full">
                          {tech}
                        </span>
                      ))}
                    </div>
                ...
                {/* Project 5: Verde Via (追加) */}
                <div className="relative">
                  <span className="absolute -left-[41px] md:-left-[57px] top-1 h-6 w-6 rounded-full border-4 border-white dark:border-gray-900 bg-lime-500"></span>
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow">
                    <div className="flex flex-col md:flex-row md:items-start gap-4 mb-4">
                      <div className="w-full md:w-32 h-20 relative flex-shrink-0 bg-lime-50 dark:bg-lime-900/20 rounded-lg flex items-center justify-center">
                         <svg className="w-8 h-8 text-lime-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      </div>
                      <div>
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white">{t.exp_5_title}</h3>
                        <p className="text-sm text-lime-600 dark:text-lime-400 font-medium mb-2">{t.exp_5_period}</p>
                        <p className="text-gray-600 dark:text-gray-300 text-sm leading-relaxed">{t.exp_5_desc}</p>
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-2 mt-4">
                      {t.exp_5_tech.map(tech => (
                        <span key={tech} className="px-3 py-1 text-xs font-medium text-lime-700 bg-lime-50 dark:bg-lime-900/30 dark:text-lime-300 rounded-full">
                          {tech}
                        </span>
                      ))}
                    </div>
                ...
                {/* Project 6: FYP (追加 - 未来の予定として少し色味を抑える、あるいは特別な色) */}
                <div className="relative">
                  <span className="absolute -left-[41px] md:-left-[57px] top-1 h-6 w-6 rounded-full border-4 border-white dark:border-gray-900 bg-indigo-500"></span>
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow">
                    <div className="flex flex-col md:flex-row md:items-start gap-4 mb-4">
                      <div className="w-full md:w-32 h-20 relative flex-shrink-0 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg flex items-center justify-center">
                      ...
                      <div>
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white">{t.exp_6_title}</h3>
                        <p className="text-sm text-indigo-600 dark:text-indigo-400 font-medium mb-2">{t.exp_6_period}</p>
                        <p className="text-gray-600 dark:text-gray-300 text-sm leading-relaxed">{t.exp_6_desc}</p>
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-2 mt-4">
                      {t.exp_6_tech.map(tech => (
                        <span key={tech} className="px-3 py-1 text-xs font-medium text-indigo-700 bg-indigo-50 dark:bg-indigo-900/30 dark:text-indigo-300 rounded-full">
                          {tech}
                        </span>
                      ))}
                    </div>
